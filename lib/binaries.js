...
module.exports = {
  /**
   * @method isElectron
   * Check if node windows is running inside electron application
   * @returns {boolean}
   */
  isElectron: function() {
    // Renderer process
  ...
  elevate: function (cmd, options, callback) {
    const electronPath = (() => path.join(__dirname.split('app.asar')[0], 'app.asar.unpacked', 'node_modules', 'node-windows', 'bin'))();
    let appPath = this.isElectron() ? electronPath : bin;
    const p = params(options, callback);
    exec(`"${path.join(appPath, 'elevate', 'elevate.cmd')}" ${cmd}`, p.options, p.callback);
  },
